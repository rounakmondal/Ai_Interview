...
function WBCSPreviewCards({
  ...
  React.useEffect(() => {
    fetch("/mock_test/wbcs/manifest.json")
      .then((r) => r.json())
      .then((d) => setPapers(d.papers ?? []))
      .catch(() => setPapers([]))
      .finally(() => setLoading(false));
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="rounded-2xl border border-teal-700/10 bg-card/60 p-5 animate-pulse">
            <div className="h-4 bg-muted rounded w-1/2 mb-4" />
            <div className="h-3 bg-muted rounded w-full mb-2" />
            <div className="h-3 bg-muted rounded w-3/4 mb-5" />
            <div className="grid grid-cols-2 gap-2 mb-4">
      ...
      {papers.map((paper, idx) => {
        ...
        return (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: idx * 0.04, ease: [0.22, 1, 0.36, 1] }}
            className="group relative flex flex-col rounded-2xl border border-teal-700/15 bg-card/80 hover:border-teal-500/40 hover:shadow-xl transition-all overflow-hidden"
          >
            <div className="h-1 w-full bg-gradient-to-r from-teal-500 via-cyan-500 to-teal-400" />
            <div className="p-5 flex flex-col gap-4 flex-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-teal-700/12 text-teal-800 dark:text-teal-300 font-medium whitespace-nowrap">
                  {paper.questions} Qs
                </span>
              </div>
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  className="flex-1 gap-1.5 text-xs border-teal-700/20 hover:bg-teal-500/8"
                  onClick={() => onDownload(toFile(paper), wbcsFolder)}
                >
                  <Download className="w-3.5 h-3.5" />
                  Download
                </Button>
                <Button
                  size="sm"
                  className="flex-1 gap-1.5 text-xs bg-teal-600 hover:bg-teal-700 text-white"
                  onClick={() => onStartTest(toFile(paper), wbcsFolder, "wbcs")}
                  disabled={navigating}
                >
                  {navigating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <><Play className="w-3.5 h-3.5" />Attempt Test</>}
                </Button>
          ...
          >
            ...
            {allTabFolderKey === "police" ? (
              ...
              <SIPreviewCards
                ...
                navigating={testNavLoading}
              />
            ) : allTabFolderKey === "wbcs-manifest" ? (
              // WBCS Prelims — manifest-backed cards from /mock_test/wbcs/
              <WBCSPreviewCards
                onStartTest={handleStartTest}
                onDownload={handleDownload}
                navigating={testNavLoading}
