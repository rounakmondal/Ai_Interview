...
export default function GovtTest() {
  ...
  const handleSavePdf = useCallback(async () => {
    const hasAccess = requestPdfAccess();
    if (!hasAccess) return;

    const { jsPDF } = await import('jspdf');
    const doc = new jsPDF();
    const title = `${config?.exam} – ${config?.fullPaper ? 'Full Paper' : (config?.subject ?? 'Mixed')}`;

    doc.setFontSize(16);
    doc.text(title, 14, 18);
    doc.setFontSize(9);
    doc.setTextColor(120);
    doc.text(`${questions.length} questions  |  Generated by MedhaHub`, 14, 26);
    doc.setTextColor(0);
    ...
    questions.forEach((q, i) => {
      ...
      const blockH = (qLines.length + q.options.length + 1) * lineH + 8;
      if (y + blockH > pageH - 10) { doc.addPage(); y = 20; }
      ...
      q.options.forEach((opt, oi) => {
        ...
        if (isCorrect) doc.setTextColor(0, 140, 60);
        const prefix = isCorrect ? `${labels[oi]}) * ` : `${labels[oi]})   `;
        const optLines = doc.splitTextToSize(`${prefix}${opt}`, maxW - 6);
    ...
    doc.save(`${(config?.exam ?? 'exam').replace(/\s+/g, '_')}_questions.pdf`);
  }, [requestPdfAccess, questions, config]);
