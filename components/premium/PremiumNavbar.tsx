              ...
              {navItems.map((item, idx) => (
                ...
                >
                  ...
                  >
                    {item.label}
                  <span 
  className="absolute bottom-1 left-4 right-4 h-[2px] rounded-full origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out" 
  style={{ background: 'linear-gradient(90deg, orange, red)' }} // এখানে গ্রেডিয়েন্ট যোগ হলো
/>
                  </Link>
                </motion.div>
              ))}
...
>
  {/* Base Gradient: 90deg Orange to Red */}
  <div 
    className="absolute inset-0 transition-all duration-300" 
    style={{ background: "linear-gradient(90deg, #f97316 0%, #ef4444 100%)" }} 
  />
  
  {/* Hover Gradient: Slightly brighter/shifted for depth */}
  <div 
    className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300" 
    style={{ background: "linear-gradient(90deg, #fb923c 0%, #dc2626 100%)" }} 
  />
...
>
  {/* 1. Base Gradient: 90deg Orange to Red */}
  <div 
    className="absolute inset-0 transition-all duration-300" 
    style={{ background: "linear-gradient(90deg, #f97316, #dc2626)" }} 
  />

  {/* 2. Hover Gradient: Brighter Orange to Red */}
  <div 
    className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300" 
    style={{ background: "linear-gradient(90deg, #fb923c, #ef4444)" }} 
  />

  {/* 3. Outer Glow: Orange Glow (Replaces Indigo) */}
                  ...
                  <>
                    ...
                    <Link to="/setup" onClick={() => setIsMobileOpen(false)}>
                      <motion.button
                        ...
                        transition={{ delay: 0.25 }}
                        whileTap={{ scale: 0.98 }}
                       className="w-full px-4 py-3 rounded-full text-white font-semibold text-[15px] flex items-center justify-center gap-2 shadow-lg shadow-orange-500/30"
                      >
                        {t("getStarted")}
